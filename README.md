...
I primarily focused on measuring the severity of outages by outage duration. Of the 1,534 original datapoints, 244 rows of the dataframe either had `OUTAGE.DURATION` values that were missing or were less than 5 minutes. I chose to exclude the rows that fit this criteria since outages under 5 minutes are trivial and I did not want to use imputed values for my regression model later in the analysis. This left 1,281 datapoints to use in the initial analysis.

## Univariate Analysis
The bar chart below shows the number of outages by climate region. The Northeast and South have the most outages which could be due to more extreme weather conditions that knock out grids or cause outages due to increased demand for climate-controlling appliances.
